$(document).ready(function() {
    ...
    doParseComplete = function(a, b) {
        return (new Date).getTime() - __ajaxStartTime,
				__resultBox.empty(),
				__resultBox.hide(),
				a.is(":visible") === !1
					? !1 : 0 === b[1].length ? !1
					: ($.each(b[1], function(a, b) {
    ...
    doParseUrl = function(a) {
        ...
        return f = "^([a-z0-9]+.)?" + d + "$",
               g = new RegExp(f), g.test(document.domain)
                    ? (h = !1, i = !1, j = {}, a.length <= 0
                        ? (isMobile.any() || $("input[name=keywords]").focus(), doShowPageMain(), !1)
                        : ($("input[type=radio][name=li][value=1]").prop("checked", !1),
                           $("input[type=checkbox]").prop("checked", !1),
                           $("input[type=hidden]").each(function() {
                                $(this).val(""),
                                $(this).parent().find(".item").removeClass().addClass("item"),
                                $(this).parent().find(".text").removeClass().addClass("default text"),
                                $(this).parent().find(".text").text($(this).parent().find('.item[data-value="none"]').text())
                            }), $.each(a.split("&"), function(a, b) {
                                var f, g, c = b.split("="), d = c[0], e = c[1];
                ...
                switch (d) {//{{{ 构造url 选项
                    ...
                    case "filter":
                        i = !0, j["filter"] = e, $("input[type=checkbox]").each(function() {
                            ...
                            h === !1 || "" == h
                                ? !1
                                : ($('input[name="keywords"]').val(h),
                                  k = $.cookie("mobsearchtimes"),
                                  null === k && (k = 0),
                                  f = new RegExp("^1([0-9]{10})$"), f.test(h.replace(/\s/g, ""))
                                    && (k++, date = new Date, date.setTime(date.getTime() + 6e5),
                                        $.cookie("mobsearchtimes", k, {
                                            expires: date,
                                            path: "/",
                                            domain: document.domain
                                        })),
                                  l = 10,
                                  -1 !== document.referrer.indexOf("3bsou.com")
                                  && (l = 5),
                                     k >= l
                                     ? (_czc.push(["_trackEvent", "error", "mobsearchtimes", "", 0, "search"]),
                                        $(".page-main").hide(),
                                        $(".page-search").show(),
                                        $(".search-loading").show(),
                                        $(".search-result").hide(),
                                        $(".search-loading").children("span.loading").hide(),
                                        $(".search-loading").children(".error").show(),
                                        $(".search-loading").children(".error").children(".content").html('<div class="header"></div><ul class="list"><li></li><li></li><li>root@googto.com</li></ul>'), !1)
                                     : (i === !0 && ($(".search-option-button").remove(), $(".search-option").show()), doSearch(h, j), void 0))))
                    : !1
    },
    ...
    setUrl = function(a, b) {
        return __ajaxSearchThread.length > 0
            && window.location.pathname + window.location.search === a
                ? !1
                : (doCNZZPageView(a), void 0 === b
            && window.history.pushState(null, null, a), a = a.replace("#", "").replace("?", ""), "/" == a.substr(0, 1)
            && (a = a.substr(1)), __setUrlAndParse === !0 && doParseUrl(a), void 0)
    },
